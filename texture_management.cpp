...
list<managed_texture *> textures;


managed_texture::managed_texture(unsigned i, const ustring &n, bool od):
...
managed_texture *add_texture(unsigned id, const ustring &name, bool ondemand)
{
...
managed_texture *load_texture(const ustring &path)
{
    // FIXME: Avoid multiple loading of the same file

    unsigned nid = load_bmp_texture(path.data());
